...
c4.metric("RSI-14", f"{last['rsi']:.1f}",
          "Sobrecompra" if last["rsi"] > 70 else ("Sobreventa" if last["rsi"] < 30 else "Normal"),
          delta_color="inverse" if last["rsi"] > 70 else "normal")
...
with tab2:
    ...
    for i, (n, p, r, ds) in enumerate(zip(
        ...
        with cols[i]:
            ...
            st.metric("R² adj (peso)", f"{r*100:.2f}%",
    ...
    if st.button(f"🚀 Ejecutar Escaneo Rápido ({mercado})", type="primary"):
        ...
        for i, activo in enumerate(lista):
            ...
            resultados.append({
                ...
                "Fuerza Media": mod_r["fuerza_media"], "R² Medio": mod_r["r2_medio"],
        ...
        if st.button("🔄 Ejecutar Auditoría en Vivo", type="primary"):
            ...
            for idx, row in df_abiertas.iterrows():
                ...
                try:
                    ...
                    estado_tiempo  = (f"⏳ Quedan {dias_restantes}d" if dias_restantes > 0
                                      else ("🚨 CERRAR HOY" if dias_restantes == 0
